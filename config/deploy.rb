...
task :deploy do
  on roles(:web) do
    ...
    if test "[ -d #{deploy_to}/#{application} ]"
      execute "cd #{deploy_to}/#{application}; git pull; sudo bash -l -x setup.sh"
    else
      execute "cd #{deploy_to}; git clone -b #{branch} #{fetch :repo_url} #{application}; cd #{application}; sudo bash -l -x setup.sh"
    end
	end
end
